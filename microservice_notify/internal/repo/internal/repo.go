...
func (p *Postgres) CreateMessage(ctx context.Context, msg models.SMSMessage) error {
	_, err := p.pool.Exec(ctx, `
		INSERT INTO sms_messages (phone_number, message, code, retry, status)
		VALUES ($1, $2, $3, $4, 'not_sent')
	`, msg.PhoneNumber, msg.Message, msg.Code, msg.Retry)
	return err
}

func (p *Postgres) GetNextMessageForGateway(ctx context.Context, gatewayType string) (*models.SMSMessage, error) {
	...
	// Определяем тип сообщений по типу шлюза
	var retryCondition string
	switch gatewayType {
	case "regular":
		retryCondition = "retry IN (1,2)"
	case "premium":
		retryCondition = "retry = 3"
	default:
		return nil, fmt.Errorf("unknown gateway type: %s", gatewayType)
	}

	query := fmt.Sprintf(`
		SELECT id, phone_number, message, code, retry
		FROM sms_messages
		WHERE status = 'not_sent' AND %s
	...
	err = tx.QueryRow(ctx, query).Scan(
		&msg.ID, &msg.PhoneNumber, &msg.Message, &msg.Code, &msg.Retry,
	)
	if err != nil {
...
func (p *Postgres) UpdateMessageStatus(ctx context.Context, id int, status, gateway string) error {
	_, err := p.pool.Exec(ctx, `
		UPDATE sms_messages
		SET status = $1, gateway = $2, updated_at = NOW()
		WHERE id = $3
	`, status, gateway, id)
	return err
}

func (p *Postgres) LogAttempt(ctx context.Context, msgID int, workerID, gateway, status, errorMsg string) error {
	_, err := p.pool.Exec(ctx, `
		INSERT INTO sms_logs (message_id, worker_id, gateway, status, error)
		VALUES ($1, $2, $3, $4, $5)
	`, msgID, workerID, gateway, status, errorMsg)
	return err
}

...
func NewPostgres(ctx context.Context, conn string) (*Postgres, error) {
	...
	// нужно проверить реальную связь с пулом, но может быть timeout поэтому подстраховываемся контекстом
...
func (p *Postgres) InsertSms(ctx context.Context, msg *models.ValidatedMsg) error {
	var id int32 // задаем явно если SERIAL
	...
	if err != nil {
		log.Printf("failed to insert SMS message: %v\nPhone: %s\nRetry: %d\n",
			err,
			msg.Phone,
			msg.Retry,
		)
		return fmt.Errorf("error inserting SMS message: %w", err)
	}

	if id <= 0 { // Явная проверка на невалидный ID
		log.Printf("unexpected inserted SMS ID: %d (phone: %s)", id, msg.Phone)
...
func (p *Postgres) InsertCall(ctx context.Context, msg *models.ValidatedMsg) error {
	var id int32 // задаем явно если SERIAL
	...
	if err != nil {
		...
		return fmt.Errorf("error inserting CALL message: %w", err)
	}

	if id <= 0 { // Явная проверка на невалидный ID
		log.Printf("unexpected inserted CALL ID: %d (phone: %s)", id, msg.Phone)
		return fmt.Errorf("invalid ID returned: %d", id)
	}
...
func (p *Postgres) GetNextSmsForGateway(ctx context.Context, gatewayType string) (*models.MsgFromRepo, error) {
	...
	// обязательно юзаем SKIP LOCKED, чтоб пропускать строки, которые заблокированы другими транзакциями!!!
	var query string
	switch gatewayType {
	case "regular":
		query = `
        SELECT id, phone, code, retry
        FROM sms_messages
        WHERE status = 'wait' AND retry IN (1,2)
	...
	msg := &models.MsgFromRepo{}
	err = tx.QueryRow(ctx, query).Scan(
		&msg.Id, &msg.Phone, &msg.Code, &msg.Retry,
	)
	if err != nil {
...
func (p *Postgres) UpdateMessageStatus(ctx context.Context, id int, status, gateway string) error {
	_, err := p.pool.Exec(ctx, `
		UPDATE sms_messages
		SET status = $1, gateway = $2
		WHERE id = $3
	`, status, gateway, id)
	return err
}

func (p *Postgres) LogSmsAttempt(ctx context.Context, msgId int, gateway, status, errorMsg string) {
	// логи это для информации только поэтому используем exec, не проверяем на returning id как с чувствительными данными
	// ошибку не нужно возвращать
	p.pool.Exec(ctx, `
		INSERT INTO sms_log (message_id, gateway, status, error)
		VALUES ($1, $2, $3, $4)
	`, msgId, gateway, status, errorMsg)
}

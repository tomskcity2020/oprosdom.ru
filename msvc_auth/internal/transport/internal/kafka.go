...
func (p *Producer) Send(ctx context.Context, msg *pb.MsgCode) error {
	...
	err = p.writer.WriteMessages(ctxTimeout, kafka.Message{
		Key:   []byte(msg.Phone), // Партиционирование по номеру телефона упрощает обработку и масштабирование, сохраняя последовательность для каждого пользователя. Если юзер несколько кодов запросит, чтоб и он и мы понимали где последний актуальный код
		Value: protoData,
	})
